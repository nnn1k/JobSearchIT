...
from backend.core.database.models.employer import CompaniesOrm, EmployersOrm, VacanciesOrm
...
async def get_employer_by_id_queries(employer_id: int, session: AsyncSession) -> EmployerResponseSchema:
    result = await session.execute(
        select(EmployersOrm)
        .options(joinedload(EmployersOrm.company).selectinload(CompaniesOrm.vacancies).joinedload(VacanciesOrm.profession))
